...
# By Mihail Igoshin added option to enable coloring and changed log dir to /opt/bh/.$USER


# To enable coloring set it to: Enable, True or 1. It's casesensitive
coloring=False
...
function prompt_command_exit() {
	trap - EXIT
	# mark the logout
	local HistFile="/opt/bh/.${USER}/bash_history/$(date '+%Y-%m/%Y-%m-%d')"
	mkdir -p "${HistFile%/*}"
	local Cmd="$(history 1)"
